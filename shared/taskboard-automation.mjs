...
export function buildTaskboardAutomationPrompt(request) {
  ...
  const executionInstructions = remoteProject
    ? [
        ...
        `认领成功后，已有完整 threadBinding 时，只能使用其保存的 threadId 和 codexHostId 调用 Codex send_message_to_thread；若保存的主机或会话不可用，使用 comment add 记录错误，再用完整保存 binding 将议题移动到 blocked 并停止，不得在当前自动化目标创建替代会话。只有未绑定议题才使用 Codex create_thread 创建远程任务，target 必须是 {type:"project",projectId:${JSON.stringify(request.codexProjectId)},environment:{type:"local"}}，其首次 identity 为 projectId=${JSON.stringify(request.codexProjectId)}、kind="remote"、hostId=${JSON.stringify(request.codexHostId)}、workspacePath=${JSON.stringify(request.workspacePath)}。发送给远程会话的指令必须包含议题编号、标题、完整描述、全部评论和开发上下文，并说明远程会话不运行 taskctl，只需完成实现、验证并返回改动、结果和剩余风险。`,
        "仅当 send_message_to_thread 成功，或 create_thread 成功返回远程 threadId，才视为远程 worker 已确认。未绑定议题在 create_thread 失败时，使用 comment add 记录失败工具和错误；随后重新 issue get，用最新 version 和 --clear-binding-thread 将当前议题移回 todo 并结束。此补偿只处理本轮当前已认领议题；不得扫描或接管其他 in_progress。",
        `新建远程任务成功后，重新 issue get，并用最新 version 再次移动到 in_progress；必须用完整 binding 参数保存 create_thread 返回的 threadId 和 projectId=${JSON.stringify(request.codexProjectId)}、kind="remote"、hostId=${JSON.stringify(request.codexHostId)}、workspacePath=${JSON.stringify(request.workspacePath)}。若绑定写入失败，远程状态不确定：使用 comment add 记录失败和远程 threadId，重新 issue get，再用同一完整 binding 将议题移动到 blocked，然后停止且不得重复派发。`,
        "使用 Codex wait_threads 等待远程会话时，目标必须使用任务保存的 threadBinding.threadId 和 threadBinding.codexHostId。wait_threads 失败、远程会话明确需要用户输入或无法继续时，使用 comment add 记录原因，重新 issue get，再用完整保存 binding 将议题移动到 blocked。远程会话完成后，使用 comment add 写入改动、验证结果、执行结果和剩余风险，再用最新 version 和完整保存 binding 将议题移动到 in_review。worker 确认后的每一次 issue move 都必须显式传完整远程 binding；不要把未完成工作标记为 in_review。",
      ]
    : [
        "每次仅处理一个 todo：选定后先用 issue get 读取最新议题内容，并用 comment list 读取全部评论。根据描述和最新评论判断是否允许开始；若其中写明等待、暂不执行或当前不应开始，立即跳过并报告，不改状态。评论也包含已完成后被打回的返工要求。",
