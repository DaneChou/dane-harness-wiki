...
export function buildTaskboardAutomationPrompt(request) {
  ...
  const executionInstructions = remoteProject
    ? [
        ...
        `认领成功后，已有完整 threadBinding 时，只能使用其保存的 threadId 和 codexHostId 调用 Codex send_message_to_thread；若保存的主机或会话不可用，使用 comment add 记录错误，再用 ownedVersion、完整保存 binding 和显式 --if-version 将议题移动到 blocked；409 冲突时立即停止，不得在当前自动化目标创建替代会话。只有未绑定议题才使用 Codex create_thread 创建远程任务，target 必须是 {type:"project",projectId:${JSON.stringify(request.codexProjectId)},environment:{type:"local"}}，其首次 identity 为 projectId=${JSON.stringify(request.codexProjectId)}、kind="remote"、hostId=${JSON.stringify(request.codexHostId)}、workspacePath=${JSON.stringify(request.workspacePath)}。发送给远程会话的指令必须包含议题编号、标题、完整描述、全部评论和开发上下文，并说明远程会话不运行 taskctl，只需完成实现、验证并返回改动、结果和剩余风险。`,
