...
export function buildTaskboardAutomationPrompt(request) {
  ...
  const executionInstructions = remoteProject
    ? [
        ...
        "仅当 send_message_to_thread 成功，或 create_thread 成功返回远程 threadId，才视为远程 worker 已确认。在确认前，若 send_message_to_thread 失败且没有成功新建远程任务，或 create_thread 失败，使用 comment add 记录失败的 Codex 工具和错误；随后重新 issue get，用最新 version 将当前议题移回 todo 并结束。若首次 issue get 有远程 threadId，移回 todo 时必须用 --thread-id 显式保留该 threadId。此补偿只处理本轮当前已认领议题；不得扫描或接管其他 in_progress。",
        "仅在新建远程任务成功后，重新 issue get，并使用最新 version 再次移动到 in_progress，同时把 --thread-id 设为 create_thread 返回的远程 threadId，以替换或保存远程绑定。若该绑定写入失败，远程状态不确定：使用 comment add 记录绑定失败和远程 threadId，重新 issue get，再用最新 version 和 --thread-id 显式保留该远程 threadId，将当前议题移动到 blocked，然后停止且不得重复派发。",
        `使用 Codex wait_threads 等待该远程会话完成或需要处理；目标必须同时传入远程 threadId 和 hostId=${JSON.stringify(request.codexHostId)}。若 wait_threads 失败，远程状态不确定：使用 comment add 记录等待失败和远程 threadId，重新 issue get，再用最新 version 和 --thread-id 显式保留该远程 threadId，将当前议题移动到 blocked，然后停止且不得重复派发。远程会话完成后，由当前本地控制器使用 comment add 写入其改动、验证结果、执行结果和剩余风险，再用最新 version 和远程 threadId 将议题移动到 in_review。若远程会话明确需要用户输入或无法继续，则记录原因并移动到 blocked；不要把未完成工作标记为 in_review。`,
      ]
    : [
        "每次仅处理一个 todo：选定后先用 issue get 读取最新议题内容，并用 comment list 读取全部评论。根据描述和最新评论判断是否允许开始；若其中写明等待、暂不执行或当前不应开始，立即跳过并报告，不改状态。评论也包含已完成后被打回的返工要求。",
