...
export class TaskboardDatabase {
  ...
  #migrate() {
    ...
    if (!taskColumns.some((column) => column.name === "execution_target")) {
      this.database.exec("ALTER TABLE tasks ADD COLUMN execution_target TEXT");
    }
    ...
    const routingMigrationTimestamp = now();
    this.database.prepare(`
      UPDATE tasks
      SET status = 'backlog', version = version + 1, updated_at = ?
      WHERE status = 'todo' AND execution_target IS NULL
    `).run(routingMigrationTimestamp);
    this.database.exec(`
      CREATE TRIGGER IF NOT EXISTS tasks_todo_execution_target_insert
      ...
      CREATE TRIGGER IF NOT EXISTS tasks_todo_execution_target_update
  ...
  syncJiraTasks(issues, { archiveMissing = true, projectName, legacyIdentity = null } = {}) {
    ...
    try {
      ...
      for (const issue of issues) {
        ...
        const persistedStatus = issue.status === "todo" && existing?.execution_target == null
          ? "backlog"
          : issue.status;
  ...
  createTask(input) {
    if (input.status === "todo" && !input.executionTarget) {
      throw new ApiError(400, "EXECUTION_TARGET_REQUIRED", "A todo issue requires an executionTarget");
    }
    this.database.exec("BEGIN IMMEDIATE");
    try {
  ...
  updateTask(id, version, changes, threadId, threadBinding, actor) {
    ...
    if (targetStatus === "todo" && !executionTarget) {
      throw new ApiError(400, "EXECUTION_TARGET_REQUIRED", "A todo issue requires an executionTarget");
    }
    if (
  ...
  moveTask(id, version, status, sortOrder, threadId, threadBinding, actor) {
    ...
    if (status === "todo" && !current.executionTarget) {
      throw new ApiError(400, "EXECUTION_TARGET_REQUIRED", "A todo issue requires an executionTarget");
    }
