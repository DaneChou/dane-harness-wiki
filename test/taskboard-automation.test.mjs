...
test("the remote automation prompt keeps taskctl local and delegates work to the SSH project", () => {
  ...
  assert.match(prompt, /存在 threadId 但没有完整 threadBinding[\s\S]*移动到 blocked[\s\S]*不得 send、create 或覆盖绑定/);
  ...
  assert.match(prompt, /create_thread 失败[\s\S]*comment add[\s\S]*--clear-binding-thread[\s\S]*移回 todo/);
