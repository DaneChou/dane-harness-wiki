...
test("issues expose processing conversations without manual binding", () => {
  ...
  assert.match(detailSource, /currentTask\.threadBinding/);
  ...
  assert.match(detailSource, /comment\.threadBinding/);
  assert.match(detailSource, /binding=\{comment\.threadBinding\}/);
